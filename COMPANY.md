...
4. Implementer がコード変更とテスト追加を行う。
5. QA Agent が受け入れ条件、画面、API、回帰を確認する。
6. Reviewer が差分をレビューし、重大リスクを指摘する。
7. 問題があれば Implementer に戻す。
8. Release Manager がリリースノート、デプロイ手順、ロールバックをまとめる。
...
- QAとReviewerは同じ観点を重複させない。QAは仕様通り動くか、Reviewerは品質と保守性を見る。
