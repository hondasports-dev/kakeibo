...
- ロール Chip、メンバー数 Chip
- 複数グループ所属時: グループ切替セレクト + 切替ボタン
- 単一グループ時: グループ名のテキスト表示
- グループ名変更フォーム（#215 まで disabled プレースホルダ）
...
- 招待メール入力 + 送信ボタン（既存）
- pending 招待一覧プレースホルダ（`pending-invites-placeholder`、#218 で実データに差し替え）
- 招待取り消し（#219 で一覧行に追加）
...
| #218 pending 一覧 | 招待管理セクションの `pending-invites-placeholder` を置換 |
