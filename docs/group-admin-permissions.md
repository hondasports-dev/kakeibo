...
| グループ削除 | 可（`owner` のみ） | 不可 | Phase2（#224 実装済み） | 論理削除。家計データは DB 上保持。Clerk / `users` は削除しない |
...
`deleteGroup` mutation と危険な操作 UI で提供する。物理削除ではなく論理削除とする。

| ルール | 内容 |
| --- | --- |
| 実行権限 | `owner` のみ（`requireGroupOwner`） |
| 削除方式 | `groups.status: "deleted"` と `deletedAt` を設定。家計データは保持 |
| pending 招待 | 削除時にすべて revoke |
| activeGroupId | 削除対象を active にしていた全メンバーへ、別の所属グループまたは `null` を設定 |
| Clerk / users | 削除しない |
| 監査ログ | `group_deleted` を記録（#225） |
| 確認 UI | 対象グループ名・論理削除・復旧不可・Clerk/ユーザー非削除を確認ダイアログで表示 |
