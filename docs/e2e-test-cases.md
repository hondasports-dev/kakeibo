...
### Issue #63: 画像入力の要件・プライバシーポリシー

| ID | シナリオ | 優先度 | カテゴリ | ファイル |
|----|---------|--------|---------|---------|
| I63-1 | 未同意時は画像読み取り前に外部API送信の同意ダイアログを表示する | P0 | privacy | ReceiptForm.test.tsx |
| I63-2 | 同意すると Convex に同意状態を保存してから画像読み取りを実行する | P0 | permission | ReceiptForm.test.tsx / users.test.ts |
| I63-3 | 同意拒否時は画像を送信せず、手入力保存へ戻れる | P1 | fallback | ReceiptForm.test.tsx |
...
#### シナリオ I63-1: 未同意時は画像読み取り前に外部API送信の同意ダイアログを表示する (P0/privacy)

**Given:** ユーザーがログイン済みで、画像外部API送信にまだ同意していない
**When:**
1. `画像から入力` セクションで画像を選択する
2. `読み取る` を押す
**Then:**
- 画像データはまだ外部APIへ送信されない
- 外部API送信、画像を長期保存しないこと、自動保存しないこと、手入力へ戻れることを説明するダイアログが表示される

#### シナリオ I63-2: 同意すると Convex に同意状態を保存してから画像読み取りを実行する (P0/permission)

**Given:** ユーザーがログイン済みで、画像外部API送信にまだ同意していない
**When:**
1. 画像読み取り前の同意ダイアログで `同意して読み取る` を押す
**Then:**
- `users.acceptReceiptImageExternalApiConsent` で承認時刻が保存される
- 保存後に画像読み取り Action が呼ばれる
- 次回以降は保存済みの同意状態を使って読み取り可否を判定する

#### シナリオ I63-3: 同意拒否時は画像を送信せず、手入力保存へ戻れる (P1/fallback)

**Given:** ユーザーがログイン済みで、画像外部API送信にまだ同意していない
**When:**
1. 画像読み取り前の同意ダイアログで `手入力する` を押す
2. 店舗名、金額、カテゴリを手入力して `保存して次へ` を押す
**Then:**
- 画像読み取り Action は呼ばれない
- 同意状態は保存されない
- 手入力保存が成功し、既存どおり店名・金額がクリアされる
...
2. `読み取る` を押す
3. 初回のみ、外部API送信の同意ダイアログで `同意して読み取る` を押す
**Then:**
