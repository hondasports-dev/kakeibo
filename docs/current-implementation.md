...
`/weeks/:weekStartDate` で週単位の支出・収入とカテゴリ別集計を確認する。既存の週次サマリー機能に加え、履歴ナビゲーションから月次サマリーと支出検索へ移動できる。
...
現段階では、LINEからの家計データ登録・更新・削除や画像AI登録を「実装済み」とは扱わない。Webhook基盤とアカウント連携が後続機能の土台である。
