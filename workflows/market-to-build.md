# 市場調査から開発までのワークフロー

## 目的

アプリ案が曖昧な状態から、市場調査と壁打ちを行い、ユーザーが作るものを決めた後に開発エージェントを起動して実装へ進める。

## 原則

- 市場調査フェーズでは実装しない。
- ユーザーが明示的に「この案で進める」と言うまで、開発フェーズへ進まない。
- 調査結果は、作るべき案、保留すべき案、避けるべき案に分ける。
- 開発フェーズでは、必要なエージェントだけを使う。
- 実装前にMVP範囲と作らない機能を確定する。

## 呼び出し方

Codexに次のように依頼する。

```text
virtual-software-company-agents/workflows/market-to-build.md を使って、
まず $research-current-market で市場調査と壁打ちをして。
アプリ案が決まるまでは実装サブエージェントを起動しないで。

私が「この案で進める」と言ったら、
Tech Lead、必要ならUX/UI Designer、Implementer、QA Agent、Reviewer、Release Managerを使って、
開発からリリース準備まで進めて。
```

## フェーズ1: 市場調査と壁打ち

担当:

- Product Lead
- `$research-current-market`

入力:

- ユーザーの関心領域
- 作りたいアプリの方向性
- 技術、予算、期間、個人開発かチーム開発か

実施内容:

- 現在の市場、競合、ユーザー課題、収益化、参入余地を調査する。
- アプリ候補を複数出す。
- 各候補を、痛みの強さ、頻度、支払い意欲、競争、実装難易度、集客しやすさで比較する。
- 最初に作るべき候補を1つ推奨する。

出力:

- 候補アプリ一覧
- 推奨案
- 推奨理由
- 避けるべき案
- 未確認リスク
- 次にユーザーが決めるべきこと

完了条件:

- ユーザーが候補を理解している。
- 推奨案と代替案が比較できる。
- 実装に進む前の質問が整理されている。

禁止事項:

- コードを書く。
- 技術設計へ進む。
- Implementer、QA Agent、Reviewer、Release Managerを起動する。

## Go / No-Go ゲート

開発フェーズへ進む条件:

- ユーザーが「この案で進める」「これを作る」「Go」など、明示的に承認する。
- MVP範囲が概ね決まっている。
- 作らない機能が明確になっている。

ユーザー承認がない場合:

- 追加調査を行う。
- 候補を絞り直す。
- 企画の壁打ちを続ける。

## フェーズ2: 要件確定

担当:

- Product Lead

実施内容:

- 対象ユーザーを確定する。
- 解く課題を1つに絞る。
- MVP機能を3から5個に絞る。
- 作らない機能を明記する。
- 成功指標を決める。

出力:

- 要件定義
- MVP範囲
- 非機能要件の初期案
- 受け入れ条件

## フェーズ3: 技術設計

担当:

- Tech Lead
- UIが重要な場合のみ optional UX/UI Designer

実施内容:

- 技術構成を決める。
- 画面、データ、API、状態管理を設計する。
- 実装タスクを分解する。
- テスト方針を決める。
- リスクと代替案を整理する。

出力:

- 技術方針
- 画面構成
- データ/API設計
- 実装タスク
- テスト方針
- リスク

## フェーズ4: 実装

担当:

- Implementer

実施内容:

- Tech Lead の設計に沿って実装する。
- 変更範囲を小さく保つ。
- 必要なテストを追加する。
- 実行した検証を記録する。

出力:

- コード変更
- テスト追加
- 検証結果
- 未解決事項

## フェーズ5: QAとレビュー

担当:

- QA Agent
- Reviewer

並列化:

- 実装が完了した後、QA Agent と Reviewer は並列で実行してよい。

QA Agent の確認:

- 受け入れ条件
- 主要フロー
- 異常系
- 回帰リスク
- リリース可否

Reviewer の確認:

- バグ
- セキュリティ
- 保守性
- テスト不足
- 既存設計との整合性

出力:

- QA結果
- レビュー指摘
- 修正が必要な項目
- リリース可否

## フェーズ6: 修正ループ

条件:

- QAまたはReviewerが重大な問題を見つけた場合。

戻し先:

- 仕様漏れ: Product Lead
- 設計問題: Tech Lead
- 実装バグ: Implementer
- UI問題: optional UX/UI Designer または Implementer

完了条件:

- 重大なQA不具合がない。
- Reviewerが承認できる。
- 未解決リスクが明示されている。

## フェーズ7: リリース準備

担当:

- Release Manager

実施内容:

- リリースノートを作る。
- デプロイ手順を整理する。
- リリース前後チェックを作る。
- ロールバック方針をまとめる。
- 未解決リスクを明示する。

出力:

- リリースノート
- デプロイ手順
- リリース前チェック
- リリース後確認
- ロールバック方針

## Coordinator の実行ルール

- 最初にこのワークフローを読み、現在フェーズを宣言する。
- 各フェーズの完了条件を満たしたら、次のフェーズへ進む。
- Go / No-Go ゲートでは、ユーザー承認なしに進まない。
- 開発フェーズに入ったら、作業を細かく分けて必要なサブエージェントへ委譲する。
- 最終回答では、現在フェーズ、完了したこと、次のアクションを明示する。

## 便利な開始プロンプト

```text
virtual-software-company-agents/workflows/market-to-build.md を使って進めて。

興味領域:
{興味領域}

制約:
{個人開発 / チーム開発 / 期間 / 技術スタック / 予算}

まず市場調査と壁打ちから始めて。
私が「この案で進める」と言うまで実装には進まないで。
```
