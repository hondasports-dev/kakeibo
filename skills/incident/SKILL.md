---
name: incident
description: Verification、Review、CI、E2E、外部サービス等でFAIL・BLOCKED・同一失敗の反復が起きたとき、事実と仮説を分離してRoot Causeを切り分け、適切なGateへ戻す。原因不明の失敗や再試行ループで使う。
license: Apache-2.0
---
...
base失敗を理由に、自分の変更で追加した別failureまで無視しない。
...
Root Causeは症状ではなく、**なぜ起き、どの条件で再発するか**まで説明する。
...
確定できない場合は `explicit blocker` と追加で必要なEvidenceを明示する。
