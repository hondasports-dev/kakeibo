...
describe("HistoryNavigation", () => {
  it("履歴メニュー内に週次・月次・検索の3リンクを表示し、週次を選択する", () => {
    ...
    expect(within(navigation).getByRole("link", { name: "支出検索" })).toHaveAttribute(
      "href",
      paths.search,
    );
    expect(within(navigation).getByRole("link", { name: "週次サマリー" })).toHaveAttribute(
  ...
  it("月次の選択日と検索のraw queryを導線に保持する", () => {
    ...
    expect(
      within(searchNavigation as HTMLElement).getByRole("link", { name: "支出検索" }),
