...
1. 依頼を分解する必要がある場合は、`.agents/roles/00-company-coordinator.md` から始める。
...
6. Tech Lead の仕様確定後、E2Eテスト設計が必要な場合はQA Agentで実装前レビューを行う。
7. 実装後にQA AgentとReviewerを使う。必要なら並列で実行してよい。
8. Release Managerは、リリース、デプロイ、ロールバック、本番影響がある場合だけ使う。
9. 市場調査では、ユーザーが明示的に案を承認するまで実装へ進まない。
10. Codexでサブエージェントを起動する場合は、直前に委譲理由と担当範囲を整理する。
