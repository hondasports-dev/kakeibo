...
- Codexでサブエージェント機能が未ロードなら、まず `tool_search` で multi-agent / spawn 系ツールを探す。`multi_agent_v1.spawn_agent` が使える場合はそれを使う。
...
- サブエージェントへ委譲する場合は、担当範囲、編集してよいファイル、成果物、検証方法、秘密情報を出力しないことを明示する。
- 役割名はプロンプトに明示する。例: `プロダクトリードA サブエージェントを起動`、`技術リード サブエージェントを起動`、`QAエージェント サブエージェントを起動`、`レビュアー サブエージェントを起動`。
- Implementer 相当のサブエージェントへ委譲する場合、作業ブランチ用の `git worktree` 作成、
  branch作成、stage、commit、push、PR作成はメインエージェントが担当し、サブエージェントには
  作業対象のパスと禁止操作を明示する。Git作業ツリーが不要な小さな変更では、現在の `cwd` と編集範囲を渡す。
