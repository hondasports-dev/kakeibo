...
- Codexでは、ユーザーが「必要に応じてサブエージェントを起動してよい」と明示した場合、それを単なる許可ではなく、外部サービス確認を独立して進められる局面で実行時サブエージェント起動を要求する指示として扱う。
- Codexでサブエージェント機能が未ロードなら、まず `tool_search` で multi-agent / spawn 系ツールを探す。`multi_agent_v1.spawn_agent` が使える場合は、`外部サービス安全確認 サブエージェントを起動` という名前をプロンプトに含め、`agent_type: qa_agent` または `default` で起動する。
- Codexで外部サービス操作のサブエージェント起動条件を満たす場合は、メインエージェントだけで代替せず、許可されたサービス、環境、実行してよい操作を限定して実行時サブエージェントを起動する。
...
- 実行時サブエージェントが利用できない環境では、利用できない理由を明記してから、メインエージェントが同じ安全確認を行う。
