...
実装前に必ず通す。フェーズ0の正本は `.agents/skills/issue-gate-0/SKILL.md`。
`issue-delivery` 開始時も、先に `issue-gate-0` を完了し、GATE0 成果物と統合判定 **Go** を得てから実装へ進む。

`issue-delivery` が使われたら、Issue の再精査として次の複数ロール確認を必須にする（詳細・出力形式は `issue-gate-0` 参照）。
